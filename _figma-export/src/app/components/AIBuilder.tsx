...
const BUSINESS_TYPES = [
  ...
  },
  {
    ...
    label: "Bookings",
    icon: CalendarDays,
    description: "Appointments & classes",
    accent: "emerald",
  },
  {
    id: "digital",
  ...
  ecommerce: [
    {
      icon: "💍",
      title: "Jewelry Brand",
      prompt: "Minimalist jewelry brand from Mexico City — Stripe checkout, abandoned cart emails, and WhatsApp order notifications",
    },
    {
      icon: "🧴",
      title: "Skincare Store",
      prompt: "Korean skincare store with curated bundles, subscription boxes, and influencer referral tracking",
    },
    {
      icon: "🏺",
      title: "Ceramics Shop",
      prompt: "Handcrafted ceramics shop with international shipping, studio visit bookings, and email newsletter",
    },
    {
      icon: "👟",
      title: "Sneaker Boutique",
      prompt: "Limited-edition sneaker boutique with drop scheduling, waitlist signup, and resell market integration",
    },
  ],
  services: [
    {
      ...
      prompt: "Boutique interior design studio with project intake, mood board sharing, and milestone-based invoicing",
    },
    {
      icon: "🎬",
      title: "Video Agency",
      prompt: "Freelance video editing agency with brief intake, revision tracking, automated contracts, and Stripe payments",
    },
    {
      icon: "📈",
      title: "Marketing Studio",
      prompt: "Growth marketing consultancy with retainer management, weekly report automation, and client-facing portal",
    },
    {
      icon: "⚖️",
      title: "Legal Firm",
      prompt: "Boutique legal consultancy with intake questionnaire, e-signature, and secure document delivery",
    },
  ],
  bookings: [
    {
      icon: "🧘",
      title: "Yoga Studio",
      prompt: "Yoga studio with live class scheduling, membership tiers, automated 24h reminders, and Zoom integration",
    },
    {
      icon: "🚗",
      title: "Car Detailing",
      prompt: "Premium car detailing service with appointment slots, mobile booking, and loyalty discount system",
    },
    {
      icon: "👨‍🍳",
      title: "Private Chef",
      prompt: "Private chef service with event intake, dietary preferences capture, and automated post-booking prep sheet",
    },
    {
      icon: "💆",
      title: "Wellness Spa",
      prompt: "Luxury wellness spa with treatment booking, gift card system, membership tiers, and therapist assignment",
    },
  ],
  digital: [
    {
      ...
      prompt: "Online course platform for designers with drip lessons, community access, and completion certificates",
    },
    {
      icon: "📷",
      title: "Stock Photos",
      prompt: "Stock photo marketplace with license tiers, instant delivery, and photographer revenue share",
    },
    {
      icon: "📝",
      title: "Notion Templates",
      prompt: "Notion templates shop with instant download, bundle deals, and affiliate commission tracking",
    },
    {
      icon: "🎵",
      title: "Sample Packs",
      prompt: "Music producer selling sample packs and presets with license management and resale prevention",
    },
  ],
  saas: [
    {
      icon: "🛠",
      title: "Dev Tool",
      prompt: "Developer tool with 14-day trial, usage metering, team seats, Stripe billing portal, and usage dashboard",
    },
    {
      icon: "✍️",
      title: "AI Writer",
      prompt: "AI writing assistant with free tier, usage credits, team collaboration, and monthly usage reports",
    },
    {
      icon: "📋",
      title: "Form Builder",
      prompt: "No-code form builder with template library, embed codes, response analytics, and webhook integrations",
    },
    {
      icon: "🔗",
      title: "Link Manager",
      prompt: "Smart link management SaaS with click analytics, A/B routing, team workspaces, and custom domains",
    },
  ],
};
...
const CAPABILITIES: Record<string, { icon: typeof Store; label: string }[]> = {
  ...
  digital: [
    ...
    { icon: Globe, label: "Delivery" },
    { icon: Workflow, label: "Affiliates" },
    { icon: BarChart3, label: "Analytics" },
  ],
  saas: [
    { icon: Layers, label: "Dashboard" },
    { icon: Users, label: "Users" },
    { icon: Workflow, label: "Onboarding" },
    { icon: BarChart3, label: "Analytics" },
...
const ACCENT_CLASSES: Record<string, { pill: string; glow: string; ring: string }> = {
  ...
  digital: {
    pill: "bg-amber-400/10 border-amber-400/20 text-amber-300",
    glow: "shadow-amber-500/10",
    ring: "border-amber-400/30",
  },
  saas: {
    pill: "bg-rose-400/10 border-rose-400/20 text-rose-300",
...
export function AIBuilder() {
  ...
  // Reset addons when type changes
  useEffect(() => {
    setAddedAddons(new Set());
  }, [selectedType]);

  const handleGenerate = () => {
    if (prompt.trim()) navigate("/generating");
  };
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white flex flex-col items-center justify-center px-6 relative overflow-hidden">
      ...
      {/* Side glows for selected type */}
      <AnimatePresence>
        {selectedType === "ecommerce" && (
          <motion.div key="glow-v" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="absolute top-1/3 left-1/4 w-72 h-72 rounded-full bg-violet-500/[0.04] blur-[80px] pointer-events-none" />
        )}
        {selectedType === "services" && (
          <motion.div key="glow-s" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="absolute top-1/3 right-1/4 w-72 h-72 rounded-full bg-sky-500/[0.05] blur-[80px] pointer-events-none" />
        )}
        {selectedType === "bookings" && (
          <motion.div key="glow-e" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="absolute bottom-1/3 left-1/3 w-72 h-72 rounded-full bg-emerald-500/[0.04] blur-[80px] pointer-events-none" />
        )}
        {selectedType === "digital" && (
          <motion.div key="glow-a" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="absolute top-1/4 right-1/3 w-72 h-72 rounded-full bg-amber-500/[0.04] blur-[80px] pointer-events-none" />
        )}
        {selectedType === "saas" && (
          <motion.div key="glow-r" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="absolute bottom-1/4 left-1/4 w-72 h-72 rounded-full bg-rose-500/[0.04] blur-[80px] pointer-events-none" />
        )}
      </AnimatePresence>
      ...
      >
        ...
        <div className="flex items-center justify-center gap-3 mb-12">
          ...
          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-white/[0.04] border border-white/[0.08] rounded-full">
            <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-[11px] text-white/40 font-medium">AI Builder</span>
          </div>
        </div>

        {/* Headline */}
        <div className="text-center mb-8">
          <h1 className="text-[36px] font-semibold tracking-tight mb-3 text-white leading-tight">
            Build your{" "}
        ...
        <div className="mb-5">
          <p className="text-[11px] font-semibold text-white/25 uppercase tracking-widest mb-3 px-1">
                ...
                <motion.button
                  ...
                  className={`flex items-center gap-2 px-3.5 py-2 rounded-xl border transition-all duration-200 ${
                    isSelected
                      ...
                      : "bg-white/[0.03] border-white/[0.07] text-white/45 hover:bg-white/[0.06] hover:text-white/65 hover:border-white/[0.12]"
                  }`}
                >
                  <Icon className={`w-3.5 h-3.5 flex-shrink-0 ${isSelected ? "" : "text-white/30"}`} />
        ...
        <div className="mb-4">
          <div
            className={`relative rounded-2xl transition-all duration-300 ${
              isFocused
                ? `shadow-2xl ${activeAccent ? activeAccent.glow : "shadow-white/5"}`
                : ""
            }`}
          >
            {/* Glow border wrapper */}
              ...
              className={`absolute -inset-px rounded-2xl transition-all duration-300 pointer-events-none ${
                isFocused
                  ? activeAccent
                    ...
                    : "border border-white/25 opacity-100"
                  : "opacity-0"
              }`}
            />
            ...
            <textarea
              ...
              value={prompt}
              onChange={(e) => {
                if (e.target.value.length <= charMax) setPrompt(e.target.value);
              }}
              ...
              className="w-full h-[148px] px-5 pt-4 pb-[56px] bg-white/[0.04] border border-white/[0.08] rounded-2xl text-[14px] resize-none focus:outline-none focus:bg-white/[0.055] transition-all placeholder:text-white/20 leading-relaxed"
            />
            ...
            <div className="absolute bottom-0 left-0 right-0 px-4 py-3 flex items-center justify-between border-t border-white/[0.05]">
              ...
              <div className="flex items-center gap-3">
                {/* Char count */}
                <span className={`text-[11px] tabular-nums transition-colors ${charCount > charMax * 0.85 ? "text-amber-400/60" : "text-white/20"}`}>
                  {charCount}/{charMax}
                </span>
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[13px] font-semibold transition-all duration-200 ${
                    prompt.trim()
                      ...
                      : "bg-white/[0.06] text-white/25 cursor-not-allowed scale-95"
                  }`}
                >
          ...
          {activeAddons && (
            ...
            >
              <div className="flex items-center gap-2 flex-wrap pt-1">
                <span className="text-[11px] text-white/25 font-semibold uppercase tracking-widest mr-1 flex-shrink-0">
                  Quick add
                </span>
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-[12px] font-medium transition-all duration-150 ${
                        added
                          ? "bg-white/[0.06] border-white/[0.1] text-white/40 cursor-default"
                          : `${activeAccent ? `${activeAccent.pill} hover:opacity-80` : "bg-white/[0.04] border-white/[0.08] text-white/50 hover:bg-white/[0.07] hover:text-white/70"}`
                      }`}
                    >
                      {added ? (
                        <span className="w-3 h-3 flex-shrink-0 text-white/30">✓</span>
                      ) : (
        ...
        <AnimatePresence>
          {activeCapabilities && (
            <motion.div
              ...
              className="mb-5"
            >
              <div className="flex items-center gap-2 px-4 py-2.5 bg-white/[0.02] border border-white/[0.05] rounded-xl">
                <Zap className="w-3 h-3 text-white/20 flex-shrink-0" />
                <span className="text-[11px] text-white/25 font-medium mr-2">Generates</span>
                    ...
                    return (
                      <div key={cap.label} className="flex items-center">
                        <div className="flex items-center gap-1.5 px-2 py-1">
                          ...
                          <span className="text-[11px] text-white/40 font-medium whitespace-nowrap">{cap.label}</span>
                        </div>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {activeExamples.map((example, i) => (
              <motion.button
                ...
                className="text-left p-4 bg-white/[0.025] border border-white/[0.06] rounded-xl hover:bg-white/[0.05] hover:border-white/[0.11] transition-all duration-150 group"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2 mb-1">
                      ...
                      <ArrowRight className="w-3 h-3 text-white/20 group-hover:text-white/50 transition-all group-hover:translate-x-0.5 flex-shrink-0" />
                    </div>
                    <p className="text-[11px] text-white/30 leading-relaxed line-clamp-2 group-hover:text-white/45 transition-colors">
                      {example.prompt}
                    </p>
                  </div>
                </div>
      ...
      <motion.div
        ...
        className="absolute bottom-7 flex items-center gap-5 text-center"
      >
